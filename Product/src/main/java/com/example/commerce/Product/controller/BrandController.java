...
public class BrandController {
    ...
    @GetMapping("/viewByBrand/{brandId}")
    ...
    //:TODO Complete this after sorting out the association part
    ...
    //:TODO Complete this after sorting out the association part
    @GetMapping("/viewByBestDeals/{discountPercentage}")
    public ResponseEntity getBrandsByBestDeals(@PathVariable(name = "discountPercentage") String discount){
        log.info("initiating endpoint to GET Brands offering best Deals with {} discount", discount);
        List<BrandResponseDto> responseDtoList = brandService.getBestDeals(discount);
        if (responseDtoList.isEmpty() || Objects.isNull(responseDtoList)){
            return new ResponseEntity<>(Collections.emptyList(), HttpStatus.NO_CONTENT);
        }
    ...
    public ResponseEntity updateBrandDetails(@PathVariable(name = "brandId")String brandId, @RequestBody BrandRequestDto requestDto){
        ...
        if(StringUtils.isEmpty(brandId.trim())){
            throw new CustomExceptions(CheckedExceptions.NO_CONTENT_AVAILABLE);
        }
        BrandResponseDto responseDto = brandService.updateBrand(Long.parseLong(brandId.trim()), requestDto);
    ...
    public ResponseEntity removeBrand(@PathVariable(name = "brandId")String brandId){
        ...
        boolean result = brandService.removeBrand(Long.parseLong(brandId.trim()));
